#!/usr/bin/env bash
# Cloudflare DNS A 레코드들을 현재 공인 IP로 유지한다 (유동 IP 대응)
# 필요: /etc/gateway/cf.env 의 CF_TOKEN (Edit zone DNS 권한)
set -euo pipefail

source /etc/gateway/cf.env
ZONE_NAME="sudosoon.org"
RECORD_NAMES=(dogroo.sudosoon.org donoti.sudosoon.org)
...
for record_name in "${RECORD_NAMES[@]}"; do
  ...
  if [[ -z "$record_id" ]]; then
    ...
    echo "IP 갱신: ${record_name} ${current_ip} → ${ip}"
  else
